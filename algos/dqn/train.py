...
def worker_test(ps, node_buffer, opt):
    ...
    #     "-workers_num:" + str(opt.num_workers) + "%" + str(opt.a_l_ratio), sess.graph)
    ...
    while True:
        ...
        ave_test_reward = sum(test_rets)/len(test_rets)
        print("---------------------------------------------------")
        ...
        #     test_vars[0]: ave_test_reward,
        #     test_vars[1]: alratio,
        #     test_vars[2]: update_frequency
        # })
        #
        # writer.add_summary(summary_str, last_learner_step)
        # writer.flush()
...
if __name__ == '__main__':
    ...
    # ------ HyperParameters ------
    opt = HyperParameters(FLAGS.env_name, FLAGS.exp_name, FLAGS.num_nodes, FLAGS.num_workers, FLAGS.a_l_ratio, FLAGS.weights_file)
