...
class ParameterServer(object):
    ...
    def pool_push(self):
        if len(self.weights_pool) >= 100:
            self.weights_pool = self.weights_pool[1:]
        self.weights_pool.append(self.weights)

    def pool_pull(self, keys):
        if np.random.random() > 0.5:
            worker_weights = self.weights_pool[len(self.weights_pool)-1]
        else:
    ...
    while True:
        ...
        while True:
            ...
            if d or (ep_len * opt.action_repeat >= opt.max_ep_len):
                ...
                print('rollout_ep_len:', ep_len * opt.action_repeat, 'our_side:', our_side, 'rollout_ep_ret:', ep_ret[our_side])
